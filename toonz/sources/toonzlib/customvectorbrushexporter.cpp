        ...
        errorMessage =
            QObject::tr(
                ...
                .arg(QString::fromStdString(
                    frameId.expand(TFrameId::NO_PAD)));
        return false;
      }
    }
...
bool prepareLevel(const TLevelP &level, const FrameIds &expectedFrameIds,
  ...
  try {
    ...
    if (!TFileStatus(destinationFolder).isWritableDir()) {
      errorMessage =
          QObject::tr("The destination folder is not writable: %1")
    ...
    {
      // Reserve a unique name, then release it before invoking the PLI writer.
      // The legacy Windows PLI stream cannot reliably reopen a path while the
      // QTemporaryFile reservation is still alive, even after close().
    ...
    if (TFileStatus(prepared.temporaryPath).doesExist()) {
      errorMessage =
          QObject::tr("Could not release the temporary PLI file.");
      removeTemporaryFile(prepared.temporaryPath);
      prepared = PreparedFile();
      return false;
    }
    ...
    if (contentHistory)
      writer->setContentHistory(contentHistory->clone());
    writer->save(level);
...
bool prepareAppend(const Source &source, const TFilePath &destinationPath,
  ...
  try {
    ...
    for (TLevel::Iterator it = existingLevel->begin();
      ...
      highestFrameNumber =
          (std::max)(highestFrameNumber, frameId.getNumber());
    ...
    return prepareLevel(
        outputLevel, expectedFrameIds, destinationPath.getParentDir(),
        reader->getCreator(), reader->getContentHistory(), prepared,
        errorMessage);
  } catch (const TException &exception) {
    ...
    errorMessage =
        QObject::tr("An unexpected error occurred while appending to the PLI "
                    "file.");
    return false;
  }
}
