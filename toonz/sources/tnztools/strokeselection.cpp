...
// Bundled vector brushes currently remain below 22 KiB. Keep this a soft
// warning so complex but intentional brushes are still possible.
...
QString validateBrushName(const QString &name) {
  ...
  if (!TFilePath(name + ".pli").getFrame().isNoFrame())
    return QObject::tr(
        "A brush name cannot end with a numbered frame suffix.");
...
QString suggestedBrushName() {
  TXshLevel *level =
      TTool::getApplication()->getCurrentLevel()->getLevel();
...
void StrokeSelection::saveSelectedVectors(bool chooseDestination) {
  ...
  if (chooseDestination) {
    ...
    QString fileName = QFileDialog::getSaveFileName(
        ...
        initialPath.getQString(),
        QObject::tr("OpenToonz Vector Level (*.pli)"), nullptr,
        QFileDialog::DontConfirmOverwrite);
    if (fileName.isEmpty()) return;

    destination = TFilePath(fileName);
    if (QString::fromStdString(destination.getType()).compare(
            "pli", Qt::CaseInsensitive) != 0)
  ...
  if (TFileStatus(destination).doesExist()) {
    ...
    const int choice =
        DVGui::MsgBox(question, QObject::tr("Overwrite"),
                      QObject::tr("Append"), QObject::tr("Cancel"), 2, parent);
  ...
  if (!CustomVectorBrushExporter::prepare(
          source, destinationFolder, prepared, errorMessage)) {
  ...
  if (installsCustomBrush &&
    ...
    const QString question =
        QObject::tr(
            ...
            .arg(sizeInKilobytes);
    if (DVGui::MsgBox(question, QObject::tr("Continue"),
                      QObject::tr("Cancel"), 1, parent) != 1) {
  ...
  if (appendToDestination &&
      !CustomVectorBrushExporter::prepareAppend(
  ...
  if (!CustomVectorBrushExporter::commit(prepared, destination,
                                         errorMessage)) {
    CustomVectorBrushExporter::discard(prepared);
    DVGui::error(errorMessage);
    return;
