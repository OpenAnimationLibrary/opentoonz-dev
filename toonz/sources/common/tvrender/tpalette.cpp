...
bool TPalette::reorderStyles(const std::vector<int> &oldToNew) {
  ...
  // Prepare every allocation before modifying live state. Moving the entire
  // style entry preserves names, links and type-specific parameters; moving
  // animation table keys preserves all keyframes, not just today's color.
