...
// Writes and validates a temporary PLI in destinationFolder. The preferred
// source frame is written as frame 1; remaining frames retain their source
// order. No preferred-frame metadata is stored in the PLI.
DVAPI bool prepare(const Source &source, const TFilePath &destinationFolder,
                   PreparedFile &prepared, QString &errorMessage);

// Preserves every frame in destinationPath and appends the source frames at
// the next available whole-number frame IDs. Existing frame IDs and frame 1
// remain unchanged.
DVAPI bool prepareAppend(const Source &source,
                         const TFilePath &destinationPath,
                         PreparedFile &prepared, QString &errorMessage);

// Atomically replaces destinationPath with a previously prepared file.
