...
void FilmstripFrames::showFrame(int index, bool center) {
  TXshSimpleLevel *level = getLevel();
  if (!level) return;
  if (m_isVertical) {
    ...
    if (center && scrollBar->maximum() > scrollBar->minimum())
      scrollBar->setValue((y0 + y1) / 2 - scrollBar->pageStep() / 2);
    else
    ...
    if (center && scrollBar->maximum() > scrollBar->minimum())
      scrollBar->setValue((x0 + x1) / 2 - scrollBar->pageStep() / 2);
    else
...
void FilmstripFrames::contextMenuEvent(QContextMenuEvent *event) {
  ...
  QAction *centerCurrentFrame =
      panelMenu->addAction(tr("Keep Current Frame Centered"));
  ...
  centerCurrentFrame->setCheckable(true);
  centerCurrentFrame->setChecked(m_centerCurrentFrame);
  ...
  connect(centerCurrentFrame, SIGNAL(triggered(bool)), this,
          SLOT(centerCurrentFrameToggled(bool)));
...
void FilmstripFrames::centerCurrentFrameToggled(bool center) {
  m_centerCurrentFrame = center;
  if (center) onFrameSwitched();
}
...
void FilmstripFrames::setOrientation(bool isVertical) {
  ...
  if (index >= 0) {
    showFrame(index, m_centerCurrentFrame);
  }
  update();
}
...
void Filmstrip::save(QSettings &settings) const {
  ...
  settings.setValue("centerCurrentFrame", m_frames->isCenterCurrentFrame());
}
...
void Filmstrip::load(QSettings &settings) {
  ...
  const bool centerCurrentFrame =
      settings.value("centerCurrentFrame", true).toBool();
  m_frames->setCenterCurrentFrame(centerCurrentFrame);
}
